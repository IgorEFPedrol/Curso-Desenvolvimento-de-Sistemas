...
namespace API_FOLHA
{
    public class Estagiario : ICalculoSalario
    {
        ...
        public Estagiario(int Cod, string Nome,
            double Horas, double Vale)
        {
            this.Cod = Cod;
            this.Nome = Nome;
            this.Horas = Horas;
        ...
        public void Holerite(double salHora)
        {
            ...
            Console.WriteLine("ESTAGIÁRIO:{0}",
                Nome);
            Console.WriteLine("Salário Bruto:{0:C2}", sb);
            double desc = Desconto(sb);
            Console.WriteLine("Desconto:{0:C2}",
                desc);
            Vale = sb * 0.10;
            Console.WriteLine("Vale:{0:C2}",
                Vale);
            double sl = sb - desc + Vale;
            Console.WriteLine("Salário Liquido:{0:C2}",
                sl);
        }
    }
}